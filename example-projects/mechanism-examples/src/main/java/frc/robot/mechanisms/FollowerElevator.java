...
public final class FollowerElevator implements Mechanism {
    ...
    private final ControlBinding lift = Controls.position(leader).feedback(leader.encoder()).pid(0.5, 0.0, 0.0);

    public final Action bottom = lift.position(0.0).clamp(travel);
    public final Action middle = lift.position(0.8).clamp(travel);
    public final Action top = lift.position(1.6).clamp(travel);
}
