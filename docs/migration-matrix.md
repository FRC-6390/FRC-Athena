...
This matrix keeps evicted legacy feature areas connected to the promoted root
workspace. It is deliberately status-oriented: every legacy feature area should
either point at implemented replacement evidence or at a preserved workspace
that still needs dedicated migration work.

Status values:

- `Implemented`: the root module has code, tests, and docs for the replacement
  shape.
- `Boundary`: the root module has a compile-tested API or adapter boundary, but
...
| Legacy Area | Legacy Evidence | Root Target | Status | Root Evidence | Next Step |
| --- | --- | --- | --- | --- | --- |
...
| HeliOS/PDLib camera adapter | `athena-helios` | preserved `athena-helios` workspace | Boundary | preserved source workspace | Migrate as `athena-vendor-helios` when PDLib coordinates and adapter API are selected. |
| ARCP Rust/JNI runtime and host dashboard | `athena-arcp` | preserved `athena-arcp` workspace | Boundary | preserved source workspace and ARCP docs under `athena-arcp/docs` | Keep separate until the Rust/JNI/native publishing flow is migrated intentionally. |
...
The V3 direction and fluent Java replacement pass are promoted to the repository
root. Future work should start from a concrete new adapter, robot use case,
HeliOS migration, ARCP migration, or dashboard workspace requirement instead of
reopening the architecture shape.
