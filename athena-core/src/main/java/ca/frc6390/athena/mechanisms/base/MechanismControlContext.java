...
public interface MechanismControlContext<T extends Mechanism>
    ...
    default double feedforwardOut(String name, double velocity) {
        ...
        if (profile != null) {
            ...
            double volts = switch (profile.type()) {
                case ARM -> profile.arm().calculate(position, velocity);
                case ELEVATOR -> profile.elevator().calculate(velocity);
                case SIMPLE -> profile.simple().calculate(velocity);
            };
            OutputType from = profile.outputType() != null ? profile.outputType() : OutputType.VOLTAGE;
    ...
    default double feedforwardOut(String name, double currentVelocity, double nextVelocity) {
        ...
        if (profile != null) {
            double position = mechanism().setpoint() + mechanism().nudge();
            double volts = switch (profile.type()) {
                case ARM -> profile.arm().calculate(position, nextVelocity);
                case ELEVATOR -> profile.elevator().calculate(nextVelocity);
                case SIMPLE -> profile.simple().calculateWithVelocities(currentVelocity, nextVelocity);
            };
            OutputType from = profile.outputType() != null ? profile.outputType() : OutputType.VOLTAGE;
            return toOutput(from, volts);
        }
