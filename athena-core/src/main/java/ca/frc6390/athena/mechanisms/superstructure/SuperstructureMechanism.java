...
public class SuperstructureMechanism<S extends Enum<S> & SetpointProvider<SP>, SP> extends SubsystemBase implements RobotSendableSystem, RegisterableMechanism {
    ...
    private void queueStateInternal(S state) {
        ...
        if (!constraint.transitionStates.isEmpty() && !guard.getAsBoolean()) {
            for (S transition : constraint.transitionStates) {
                stateMachine.queue(transition);
            }
        }
        stateMachine.queue(state, guard);
    }
    ...
    private void forceSetpointInternal(SP setpoint) {
        if (setpoint == null) {
            return;
        }
    ...
    private void forceStateInternal(S target) {
        if (target == null) {
            return;
        }
        queuedSetpoint = null;
    ...
    private void ensureNetworkTableCommandEntries(RobotNetworkTables.Node commands) {
        ...
        ntRequestButton = createCommandButton(
                commands.child("Force"),
                "Force",
                Commands.runOnce(this::forceFromChooserOrCustom).ignoringDisable(true));
    ...
    private void forceFromChooserOrCustom() {
    ...
    private boolean forceCustomSetpoint() {
        SP parsed = parseCustomSetpoint(readCustomSetpointInput());
        if (parsed != null) {
            forceSetpointInternal(parsed);
            return true;
        }
        return false;
    }
    ...
    public static final class StateMachineSection<S extends Enum<S> & SetpointProvider<SP>, SP> {
        ...
        public StateMachineSection<S, SP> force(S target) {
            owner.forceStateInternal(target);
            return this;
        }
        ...
        public boolean atGoal() {
            return owner.stateMachine.atGoal();
        }

        @SafeVarargs
        public final boolean at(S... states) {
            return owner.stateMachine.at(states);
        }
