...
Athena 2027 keeps release metadata centralized so quick revisions do not require
...
Run the full 2027 build before publishing:
...
```text
2027/vendordeps/FRC6390-Athena.json
```
...
2. Run `./gradlew -p 2027 generateVendordep build`.
3. Run `./gradlew -p 2027 releaseChecklist` for a local staging publish.
4. Inspect `2027/build/staging-repo`.
